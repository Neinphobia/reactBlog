...
const App = () => {
  ...
  const handleSubmit = async (event) => {
    ...
    try {
      setPending(true);
      await axios.post("https://api-for-react-neinphobia.vercel.app/blogs", {
        name,
        body,
        img,
        ytlink,
  ...
  const handleDelete = async (id) => {
    try {
      setPending(true);
      await axios.delete(
        `https://api-for-react-neinphobia.vercel.app/blogs/${id}`
      );
      const result = await axios.get(
        "https://api-for-react-neinphobia.vercel.app/blogs"
      );
        ...
        {data.map((item) => (
          <li key={item._id} className="data-item">
            <div className="item-header">
              {item.img && item.img.includes("https://") ? (
                <img src={`${item.img}`} alt="yüklenemedi veya fotoğraf yok." />
              ) : null}

              <br />

              {item.ytlink && item.ytlink.includes("youtu") ? (
                <a href={`${item.ytlink}`} target="_blank">
                  {item.ytlink}
                </a>
              ) : null}
